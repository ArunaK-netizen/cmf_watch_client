"""
Cryptographic primitives for Nothing X / CMF Watch BLE protocol.
AES-128-CBC encryption and decryption with fixed IV and PKCS7 padding.
"""

from cryptography.hazmat.primitives.ciphers import Cipher, algorithms, modes
from cryptography.hazmat.primitives import padding

# Fixed IV derived from Nothing X app decompilation and CmfCharacteristic
AES_IV = bytes([
...
def encrypt_aes_cbc(data: bytes, key: bytes, iv: bytes = AES_IV) -> bytes:
    """
    Encrypt data using AES-128-CBC with PKCS7 padding and fixed IV.
...
def decrypt_aes_cbc(ciphertext: bytes, key: bytes, iv: bytes = AES_IV) -> bytes:
    ...
    if len(ciphertext) == 0 or len(ciphertext) % 16 != 0:
        raise ValueError(f"Ciphertext length must be a non-zero multiple of 16, got {len(ciphertext)} bytes")
