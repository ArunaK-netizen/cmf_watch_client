"""
Framing, fragmentation, MTU chunking, and deframing for CMF / Nothing Watch BLE protocol.
"""
...
def build_frames(
    ...
    if len(payload) == 0:
        # Empty payload single chunk
        chunk_data = b"" if is_plaintext or session_key is None else b""
...
class FrameAssembler:
    ...
    def process_raw_notification(
        ...
        """
        if len(raw_bytes) < 11:
            raise ValueError(f"Raw notification byte length {len(raw_bytes)} is less than 11-byte header")
        ...
        if header != PAYLOAD_HEADER:
            raise ValueError(f"Invalid frame header byte {header:#04x}, expected {PAYLOAD_HEADER:#04x}")
        ...
        if payload_len > 0:
            if is_plaintext or session_key is None:
                ...
                if len(decrypted) < 4:
                    raise ValueError(f"Decrypted payload length {len(decrypted)} is smaller than 4-byte CRC")
                payload = decrypted[:-4]
                expected_crc = decrypted[-4:]
                if not verify_crc32(payload, expected_crc):
                    raise ValueError(f"CRC verification failed for opcode ({cmd1:#06x}, {cmd2:#06x})")
        else:
            payload = b""
