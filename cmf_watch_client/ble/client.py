"""
Asynchronous BLE Client for CMF / Nothing Smartwatches powered by Bleak.
...
from ..exceptions import CmfAuthError, CmfConnectionError, CmfProtocolError, CmfTimeoutError
...
logger = logging.getLogger("cmf_watch_client.ble")
...
class CmfWatchClient:
    ...
    def __init__(
        ...
        phone_model: str = "CMF Watch Client",
    ):
        """
        ...
        Args:
            ...
            phone_model: Device identifier string sent during handshake.
        """
        self.target = device_address_or_ble_device
        self.authkey = authkey
        self.session_key: Optional[bytes] = authkey
    ...
    async def connect(self, timeout: float = 20.0) -> bool:
        ...
        try:
            ...
            raise CmfConnectionError(f"Failed to establish GATT connection: {e}") from e

        if not self._client.is_connected:
            raise CmfConnectionError("GATT client connection attempt failed")
        ...
        try:
            ...
            raise CmfConnectionError(f"Failed to subscribe to command notify characteristic: {e}") from e
    ...
    async def pair_first_time(self) -> bytes:
        ...
        if not self._client or not self._client.is_connected:
            raise CmfConnectionError("Client must be connected before pairing")
        ...
        try:
            ...
            raise CmfTimeoutError("Timed out waiting for 'AT GETSECRET' response from watch")

        if "GETSECRET:" not in shell_reply or ",OK" not in shell_reply:
            raise CmfAuthError(f"Unexpected secret response from watch: '{shell_reply}'")

        secret_hex = shell_reply.split("GETSECRET:")[1].split(",OK")[0].strip()
        self._secret = bytes.fromhex(secret_hex)
        logger.info("Successfully obtained 16-byte device secret from watch.")
        ...
        if len(reply_payload) < 48:
            raise CmfAuthError(f"AUTH_PAIR_REPLY payload length {len(reply_payload)} is invalid (expected >= 48)")
        ...
        if signed2 != expected_signed2:
            raise CmfAuthError("Watch challenge signature mismatch during pairing!")
    ...
    async def authenticate_session(self) -> bool:
        ...
        """
        if not self.authkey:
            raise CmfAuthError("No authkey available. Call pair_first_time() first.")
    ...
    async def send_command(
        ...
        """
        if not self._client or not self._client.is_connected:
            raise CmfConnectionError("Client is not connected")
        ...
        if wait_for_reply and fut:
            try:
                ...
                raise CmfTimeoutError(f"Timed out waiting for response to opcode {target_reply}")
            finally:
